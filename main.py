...
def tab(screen, size, records, score):
    ...
    for i in range(11):
        ...
        screen.blit(rnt, (size[0] + 20, 55 + i * 50))
...
if __name__=='__main__':
    ...
    list[i1][j1] = 3
    lis_z = [[0, 2], [0, 1], [0, 0]]
    screen = pygame.display.set_mode([size[0] + 350, size[1]])
    running = True
    x = 1
    y = 0
    drawline(screen, size)
    drawzmei(screen, size, list)
    tab(screen, size, records, score)
    ti = time.time()
    while running:
        t = False
        ...
        if t or time.time() - ti >= 0.75:
            ...
            if list[(g[0] + y) % (h // 50)][(g[1] + x) % (w // 50)] == 3:
                ...
                print("Поражение")
                records.append([str(random.choice(range(1000, 10**8))), str(score)])
